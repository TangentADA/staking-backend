...
const harvestTX = async (prisma: PrismaClient, harvestReq: any) => {
  ...
  let poolIndex = harvestReq.poolIndex as number[];
  let address = harvestReq.address
  let quantity = harvestReq.quantity ? harvestReq.quantity : 1
  console.log(poolIndex)
  console.log(address)

  const pools = poolIndex.filter(item => item >= 0);
  if (pools.length == 0) return {error: "Failed, pool doesn't exist in db."}

  const poolargs = pools.map(pool => stakingPools[pool]);
  ...
  let updatePay = 0;
  for (let i = 0; i < poolargs.length; i++) {
    let pool;
    console.log(`PoolI: ${poolargs.join(", ")}`)
    try {
    ...
    if (lastUpdate.valueOf() + 300000 < Date.now()) {
      ...
      updatePay = updatePay + (epochRatio * (quantity * (poolargs[i].poolInfo.rewardPerEpochQt)))

      let valueAdded: Assets = {}
      valueAdded[poolargs[i].poolInfo.harvestUnit] = BigInt(updatePay)

      await dbFromPool(pools[i], poolargs, valueAdded)

    }

    updatePay = Math.floor(updatePay);
    ...
    let payOut: Assets = {}
    poolargs.forEach((poolItem) => {

      let payout = 0n;

      assets.forEach((a: any) => {
        if (a.unitUnit == poolItem.poolInfo.harvestUnit) {
          payout += BigInt(a.value)
        }
      });

      payOut[poolItem.poolInfo.harvestUnit] = payout;
    })
    ...
    for (let poolItem of poolargs) {
      let beUtxos = await lucid.utxosAtWithUnit(poolItem.poolInfo.distAddress, poolItem.poolInfo.harvestUnit)
      ...
      if (beLength < 10) { // Note: if statement was commented out when testing successfully with ADAO (only else statement) got triggered
        ...
        tx = tx.payToAddress(poolItem.poolInfo.distAddress, splitAssets[0])
        tx = tx.payToAddress(poolItem.poolInfo.distAddress, splitAssets[1])
      } else {
        tx = tx.payToAddress(poolItem.poolInfo.distAddress, distPayback)
      }
    }
